...
const HPKE_SUITE = new CipherSuite({
  kem: new DhkemX25519HkdfSha256() as any,
  kdf: new HkdfSha256() as any,
  aead: new Aes256Gcm() as any,
});
...
export async function hpkeEncrypt(recipientPublicKey, data, aad = new Uint8Array()) {
  ...
  const ciphertext = await sender.seal(plaintext.buffer, aad.buffer);
...
export async function hpkeDecrypt(recipientKeyPair, message, aad = new Uint8Array()) {
  try {
    ...
    const plaintext = await recipient.open(message.ciphertext.buffer || message.ciphertext, aad.buffer);
    ...
    throw new Error(`HPKE decryption failed: ${message}`);
  }
}
