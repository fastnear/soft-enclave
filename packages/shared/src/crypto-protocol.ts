...
export async function deriveSessionWithContext(privateKey: CryptoKey, publicKey: CryptoKey, ctx: any) {
  // Get our own public key from the private key's key pair
  // We need to extract it from somewhere - but we don't have access to it here
  // This is a limitation of the legacy API

  // The legacy API doesn't provide selfPublicKeyRaw, so we can't use the new API properly
  // We'll need to perform ECDH directly without transcript binding for legacy compatibility
